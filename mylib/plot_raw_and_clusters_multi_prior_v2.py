...
def fixed_box_xyxy_from_center(cx: float, cy: float, L: float, W: float) -> np.ndarray:
    """Axis-aligned box [xmin,ymin,xmax,ymax] from center and L/W (yaw assumed 0 for IoU/plot)."""
...
def plot_raw_and_clusters(
    ...
    fixed_box_fit_mode: str = "center",  # NEW: "center" or "edge"
    # edge-loss extra
    ...
    for cid in u_ids:
        ...
        if use_fixed_box:
            ...
            ax_clu.text(xmin, ymin + h, f"ID:{int(cid)}  V:{mean_v:.1f}  P:{prior_id}", fontsize=8)
        else:
