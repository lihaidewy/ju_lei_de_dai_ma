...
from cv_tracker import update_cv_track
...
def _build_predicted_position_map_from_tracks(tracks, fid):
    """
    从已有 tracks 中预测当前帧 fid 的参考位置。
    只用于关联参考，不修改轨迹内部状态。
    """
    predicted = {}
    for gid, trk in tracks.items():
        try:
            pred = trk.predict_to(fid)
        except Exception:
            pred = getattr(trk, "output_center", None)
...
def process_one_target_global_y_prob_weighted(
    ...
    if estimation_mode == "raw":
    ...
    elif estimation_mode == "cv_recursive":
        output_xy, used_measurement = update_cv_track(
            tracks=tracks,
            gid=gid,
            frame_id=fid,
            z=raw_weighted_z,
        ...
        raise ValueError(f"未知 ESTIMATION_MODE={estimation_mode}，仅支持: raw / cv_recursive")
    ...
    if output_xy is None:
        ...
        z_raw_x = float(output_xy[0]) if estimation_mode == "raw" else (
            np.nan if raw_weighted_z is None else float(raw_weighted_z[0])
        )
        z_raw_y = float(output_xy[1]) if estimation_mode == "raw" else (
            np.nan if raw_weighted_z is None else float(raw_weighted_z[1])
        )
...
def run_global_y_prob_weighted_analysis(radar_data, gt_df, frame_ids, cluster_label_map, prob_df, params):
    ...
    for fid in frame_ids:
        ...
        if str(params.get("ASSOC_REFERENCE", "gt_center")).lower() == "predicted":
            predicted_positions = _build_predicted_position_map_from_tracks(tracks, fid)
        ...
        for gt_row in gt_frame_df.itertuples(index=False):
            ...
            if np.isfinite(row["xy_error"]):
                print(
                    ...
                    f"used_meas={row['used_measurement']}, "
                ...
                print(
                    f"gid={row['gid']}, cluster={row['main_cluster']}, "
                    f"n_cluster={row['n_cluster']}, n_sel={row['n_selected']}, "
                    f"w_sum={row['weight_sum']:.3f}, used_meas={row['used_measurement']}, "
                    f"x_hat=NaN, y_hat=NaN"
                )
