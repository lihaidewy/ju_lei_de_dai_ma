...
def build_cluster_label_map(radar_data, frame_ids, cfg):
    """
    对每个有效帧执行聚类，保存“点 -> cluster label”的映射。
    """
    cluster_label_map = {}
...
def estimate_directional_speed_priors_from_gt(gt_df, frame_ids, params):
    ...
    for _, sub in use_df.groupby("ID"):
...
def main():
    ...
    if params.get("SAVE_SELECTED_MEAS_CSV", False):
        meas_csv_path = params.get(
            "SELECTED_MEAS_CSV_PATH", "selected_measurement_points.csv"
        )
