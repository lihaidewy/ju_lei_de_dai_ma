...
def _compute_global_limits(frame_ids, frame_cache):
    """
    当没有手动设置显示范围时，自动扫描所有帧得到一个全局固定范围。
    """
    all_x = []
    all_y = []
...
def _estimation_mode_text(params):
    mode = str(params.get("ESTIMATION_MODE", "kalman")).lower()
    if mode == "kalman":
        return "Kalman CV"
    if mode == "raw":
        return "Raw Measurement"
    if mode == "cv_fit":
        return "Sliding-Window CV Fit"
    return mode
...
def _draw_right_targets(ax, targets, params, show_roi_pts=True, show_gt=True, show_est=True):
    estimation_mode = str(params.get("ESTIMATION_MODE", "kalman")).lower()
...
def render_frame(fig, axes, frame_ids, state, frame_cache, params):
    ...
    #     "[x] right clusters  [z] right noise  [k] cluster text\n"
    #     "[g] right GT  [e] right estimates  [r] right ROI pts\n"
    #     "[1] left meas  [2] left gt box  [3] left ROI band  [4] left ROI pts  [5] left gt id"
    # )
    # ax_right.text(
    #     0.02,
    #     0.98,
