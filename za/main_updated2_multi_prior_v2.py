...
from mylib.plot_raw_and_clusters_multi_prior_v2 import plot_raw_and_clusters, choose_best_fixed_box_prior_mode
from mylib.plot_gt_main import load_gt_reference
from mylib.eval_clusters2_multi_prior_v2 import eval_one_frame_target_level, GT_DIM
...
def main():
    ...
    def render_frame(i: int):
        ...
        # cluster center（与 fit_mode 保持一致）
        cluster_centers = {}
    ...
    if len(all_center_err) > 0:
        ...
        print(
            ...
            f"mean={float(np.mean(ce)):.3f}  median={float(np.median(ce)):.3f}  "
            f"p90={float(np.percentile(ce, 90)):.3f}  p95={float(np.percentile(ce, 95)):.3f}"
        )
    else:
